...
def corpus(tmp_path, stem="arctic_a0001") -> Path:
...
def _train_stem() -> str:
    """A file stem whose seeded split lands on 'train'."""
    for i in range(50):
        stem = f"arctic_a{i:04d}"
        if (
            assign_split(Utterance(f"l2arctic-ABA-{stem}", Path("x"), "", "arabic", ()), 42)
            == "train"
        ):
            return stem
    raise AssertionError("no train stem found")
...
def _test_stem() -> str:
    for i in range(50):
        ...
        if (
            ...
            return stem
    raise AssertionError("no test stem found")


def test_exports_only_alphabet_classes(tmp_path):
    out = tmp_path / "out"
    manifest = export(ExportOptions(corpus(tmp_path, _train_stem()), out, val_fraction=0.0))
...
def test_held_out_utterances_are_never_exported(tmp_path):
    out = tmp_path / "out"
    manifest = export(ExportOptions(corpus(tmp_path, _test_stem()), out, val_fraction=0.0))
...
def test_val_assignment_is_per_utterance(tmp_path):
    out = tmp_path / "out"
    manifest = export(ExportOptions(corpus(tmp_path, _train_stem()), out, val_fraction=1.0))
...
def test_max_per_class_caps_every_class(tmp_path):
    out = tmp_path / "out"
    manifest = export(
        ExportOptions(corpus(tmp_path, _train_stem()), out, val_fraction=0.0, max_per_class=1)
    )
