...
def runs_and_slopes(sem, t0, hop, win_s=0.25, gap_s=0.04):
    pts = [(t0 + i*hop, v) for i, v in enumerate(sem) if v is not None]
    if len(pts) < 6: return None
    runs = []
    cur = [pts[0]]
    for p in pts[1:]:
        if p[0] - cur[-1][0] > gap_s + hop:
            runs.append(cur); cur = [p]
        else:
            cur.append(p)
    runs.append(cur)
    s_min = s_max = None; t_min = t_max = None
    for run in runs:
        if len(run) < 4: continue
        xs = [x for x, _ in run]; ys = [y for _, y in run]
        for i in range(len(run)):
            for j in range(i+3, len(run)):
                if xs[j] - xs[i] > win_s + 1e-6: break
                s = slope_of(xs[i:j+1], ys[i:j+1])
...
for unit in corpus.units():
    ...
    f = runs_and_slopes(t.semitones, t.start_s, t.hop_s)
...
print("cal best:", best)
for th in (best[1], best[1]+4, max(4, best[1]-4)):
